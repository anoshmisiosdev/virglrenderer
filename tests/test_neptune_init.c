...
test_neptune_get_capset(void)
{
   ...
   /* Backend-capability bits derive from NPT_BACKEND: the five DXMT bits
    * together for "dxmt", none for d3dmetal (default).  Mask out the
    * D3D12 bit -- it comes from a dlopen probe that varies by host. */
   const uint32_t backend_bits =
   ...
   EXPECT((caps.caps_flags & backend_bits) == 0); /* default = d3dmetal */

   setenv("NPT_BACKEND", "dxmt", 1);
   virgl_renderer_fill_caps(VIRTGPU_DRM_CAPSET_NEPTUNE, 0, &caps);
   EXPECT((caps.caps_flags & backend_bits) == backend_bits);

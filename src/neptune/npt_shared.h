 ...
 * the host D3D library and exported as a dmabuf; EXPORT_BLOB stages
 * {fd, size} as this context's pending blob under a guest-chosen
 * blob_id, which the guest KMD claims via RESOURCE_CREATE_BLOB(HOST3D,
 * blob_id).  That binds a VM-global virtio res_id to the dmabuf.  A
 * consumer context receives the fd through the proxy's attach-
 * forwarding (guest KMD CTX_ATTACH_RESOURCE) and OPEN_RES rebuilds a
 * DxvkSharedTextureDescriptor around it — zero-copy, the imported
 * texture aliases the exporter's memory.  No broker: the canonical fd
 * table is the VMM-side virgl resource table, keyed by res_id.
 */
 ...
 * \p texture_id (a guest object id in \p ctx) as a dmabuf, stage it as
